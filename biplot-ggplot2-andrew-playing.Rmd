...
This script illustrates how to create a variety of isotope biplots using ggplot2 functions.
...
# only needed to simulate the data in this example
# If you have it installed you dont need to load the 
# entire package here as i call it directly 
# below using pkg::fun notation
#library(SIBER) 
...
# for this example I am going to simulate some data using some of the SIBER
# functions.
set.seed(5)
mydata <- SIBER::generateSiberCommunity(n.groups = 3, 
                                        n.obs = 20,
                                        mu.range = c(-10,10,-10,10),
                                        wishSigmaScale = 0.1)

# rename the column headers
# NB the "community" column is not needed in this example
# but could be used to draw multiple panels via 
# facet_wrap
colnames(mydata) <- c("d13C","d15N","Taxon","community")

# coerce Taxon to be factor
...
first.plot <- ggplot(data = mydata, aes(d13C, d15N)) +
  ...
  scale_fill_viridis(discrete = TRUE, begin = 0, end = 0.9)
print(first.plot)
...
ellipse.plot <- first.plot + 
  stat_ellipseSIBER(aes(group = Taxon, 
                   fill = Taxon, 
                   color = Taxon), 
               alpha = 0.3, 
               level = p.ell,
...
Plot the data using the original SIBER method for comparison

```{r plot-siber}

mydata.2 <- mydata
colnames(mydata.2) <- c("iso1", "iso2", "group", "community")

siber <- SIBER::createSiberObject(mydata.2)

SIBER::plotSiberObject(siber, 
                       hulls = FALSE, ellipses = FALSE)

SIBER::addEllipse(siber$ML.cov[[1]][,,1], 
                  siber$ML.mu[[1]][,,1], 
                  m = siber$sample.sizes[1], 
                  small.sample = TRUE, 
                  p.interval = p.ell, 
                  ci.mean = FALSE)
...
Compare with the stat_ellipse() function in ggplot which does the maths a little differently, including using the `qf()` rather than `qchisq()` to scale the ellipses.
```{r stat-ellipse-original}
ellipse.plot.2 <- first.plot + 
...
If the permil symbol `r "\u2030"` is not showing correctly (and instead is printing as $\text{\u2030}$ in your plot it is likely because your computer is not set up to use UTF-8 format character encoding. 

Since writing the content below, I have wondered if R is simply inheriting the locale setting from your operating system, and perhaps you can override ride from with in R using the function `Sys.setlocale`. I havent checked this, but suggest you try to change this first, and failing that you can change your OS region/locale settings. Something like `Sys.setlocale(category = "LC_ALL", locale = "en")` might work, but again, I havent checked.

This is not a problem with your setup of R or Rstudio, but is deeper in your computer. It is fixed by changing the region or locale settings on your computer.You can access the region and locale settings in the system preferences area of your computer's operating system. To check if your computer is set up to identify and interpret UTF-8 encoding, you can type `sessionInfo()` in the R console. You should see something like this: `en_IE.UTF-8/en_IE.UTF-8/en_IE.UTF-8/C/en_IE.UTF-8/en_IE.UTF-8` under the heading "locale". On my machine, this indicates that it is using english (en), for Ireland (IE) and the UTF-8 encoding format. If the UTF-8 format is missing from your `sessionInfo()` then you could try changing your operating system to a locale similar to your own region's location and see if the UTF-8 format is supported there. More specifically, calling `l10n_info()` should yield `$UTF-8 [1] TRUE` indicating that this encoding is supported.
